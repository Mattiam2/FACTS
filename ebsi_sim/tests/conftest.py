...
from ebsi_sim.src.main import app
...
def client_fixture(test_engine):
    ...
    patch_middleware = patch("ebsi_sim.src.main.engine", test_engine)

    patch_lifespan = patch("ebsi_sim.src.repositories.engine", test_engine)

    test_settings = Settings(JWT_VERIFY_EXP=False)

    patch_settings_auth = patch("ebsi_sim.src.core.auth.settings", test_settings)
    patch_settings_auth_services = patch("ebsi_sim.src.services.authorisation.settings", test_settings)
